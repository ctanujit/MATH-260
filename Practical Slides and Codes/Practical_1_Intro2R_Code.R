...
ev_od_check <- function(mu, sig){
  x = round(rnorm(1, mu, sig))
  if (x %% 2 == 0){
    out <- "It is even"
  }else{
    out <- "It is odd"
...
ev_od_check_vec <- function(n, mu, sig){
  x = round(rnorm(n, mu, sig))
  out <- c()
  for(i in 1:n){
    if (x[i] %% 2 == 0){
      out[i] <- "It is even"
...
ev_od_check_vec(10, 50, 10)

die_play <- function(){     ## Lexical Scoping
  x1 <- rbinom(1, 5, p =1/6) + 1
  x2 <- rbinom(1, 5, p =1/6) + 1
  val <- x1 + x2
  if(val > 10 & x1 == x2){
    out <- "Congartulations, you won the game!"
  }else if(val <= 10 & val > 5 & x1 == x2){
    out <- "You still have a winning chance"
  }else if(val <= 5 & x1 == x2){
    out <- "You can play once more"
  }else{
    out <- "Sorry, you lost the game!"
  }
  return(out)
}
die_play() 
